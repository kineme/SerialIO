...
int openDevice(NSString *dev, int baudRate, int parity, int dataBits, int stopBits)
{
	...
	if(deviceDictionary != nil)
		...
		{
			NSLog(@"KinemeSerialIO: openDevice:  using alread-opened device for %@",dev);
	...
	NSLog(@"KinemeSerialIO: openDevice:  all finished up!");
	return serialDevice;
}
...
int configDevice(int dev, int baudRate, int parity, int dataBits, int stopBits)
{
	...
	{
		NSLog(@"KinemeSerialIO: configDevice: Error setting baud rate to %i",speed);
		//return NO;
	}
	NSLog(@"KinemeSerialIO:  %i baud rate configured",speed);
	if(tcgetattr(serialDevice, &options) == -1)
	{
		NSLog(@"KinemeSerialIO: configDevice:  Error getting attributes: %s",strerror(errno));
		//return NO;
	}
	...
	{
		NSLog(@"KinemeSerialIO: configDevice: Error configuring device: %s",strerror(errno));
		return 0;
		//return -1;
	}
